...
class Produit_core {
	...
	function modifier_produit($Produit,$id){
		$sql="UPDATE produit SET prix=:prix,quantite=:quantite,nom=:nom,image=:image,description=:description,idcat=:idecat WHERE id=:id";
...
try{		
    ...
    {
        $sql="SELECT * from produit where nom like '%$valeur%'";
        $db = config::getConnexion();
        try{
        $liste=$db->query($sql);
